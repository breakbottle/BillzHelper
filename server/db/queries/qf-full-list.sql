...
Select * from (
                ...
                Union all
                Select 'CURRENT PERIOD','',null,(select DATE_FORMAT(StartDate,'%b %d %Y %h:%i %p') StartDate from bill_creditors where id=1),false,0
                Union all
                ...
                where
                  ...
                  DATE_FORMAT(DATE_ADD( (select StartDate from bill_creditors where id=1) , INTERVAL 29 DAY ),'%b %d %Y %h:%i %p'),false,0
                Union all
                Select
                  'TOTAL',
                  'DUE NOW!!',
                  ...
                  DATE_FORMAT(DATE_ADD( (select StartDate from bill_creditors where id=1) , INTERVAL 14 DAY ),'%b %d %Y %h:%i %p'),false,0
                Union all
                Select
                  'TOTAL',
                  'DUE NEXT?',
                  ...
                  DATE_FORMAT(DATE_ADD( (select StartDate from bill_creditors where id=1) , INTERVAL 28 DAY ),'%b %d %Y %h:%i %p'),false,0
              ) fullListandCalac
order by due_date